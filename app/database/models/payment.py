...
class Payment(Base, TimestampMixin):
    ...
    Связи с подпиской и клиентом объявлены как ``SET NULL``, а не ``CASCADE``:
    удаление подписки не должно стирать историю денег. Но в рантайме это пока
    декларация — ``PRAGMA foreign_keys`` у соединения выключен, поэтому SQLite
    ограничения не применяет. Включение — отдельная задача: оно начнёт
    применять FK по всей схеме, где сейчас каскады держатся на стороне ORM.
    """
